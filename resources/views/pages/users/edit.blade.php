...
@section('content')
	...
	<div id="card" class="overflow-hidden max-w-xs p-4 bg-white rounded-b-lg border shadow-lg sm:p-6 dark:bg-gray-800 dark:border-gray-700">
		<form action="{{ route('users.update', $user->id) }}" id="formcategory" method="POST">
			@csrf
			@method('PUT')
			<div class="relative z-0 w-full mb-6 group">
				<input type="text" id="name" name="name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required="" value="{{ old('name', $user->name) }}">
			...
			<div class="relative z-0 w-full mb-6 group">
				<input type="text" name="username" id="username" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required="" value="{{ old('username', $user->username) }}">
			...
			<div class="relative z-0 w-full mb-10 group">
				<select name="is_admin" id="is_admin" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required="">
					<option value="1" @if(old('is_admin', $user->is_admin == true)) selected @endif>Admin</option>
					<option value="0" @if(old('is_admin', $user->is_admin == false)) selected @endif>Kasir</option>
				</select>
