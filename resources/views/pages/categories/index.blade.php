...
@section('content')
	...
	<h3 class="text-2xl mb-4">Kategori</h3>

	<div class="flex justify-between gap-6">
		<div>
			<div class="overflow-hidden p-2 px-5 bg-sky-400 rounded-t-lg text-white text-2xl w-64">
				Tambah kategori
			</div>
			<div id="card" class="overflow-hidden p-4 bg-white rounded-b-lg border shadow-lg sm:p-6 dark:bg-gray-800 dark:border-gray-700 w-64">
				...
				@slot('tbody')
					@foreach ($categories as $category)
						<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
							...
							<th scope="row" class="px-6 py-2 font-medium text-gray-900 dark:text-white whitespace-nowrap">{{ $category->name }}</th>
							<td class="px-6 py-2 text-right flex gap-2 justify-end items-center">
								<form action="{{ route('categories.destroy', $category->id) }}" method="POST">
									@csrf
									@method('DELETE')
									<a href="{{ route('categories.edit', $category->id) }}" class="text-white bg-green-500 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-2 py-1 dark:bg-green-600 dark:hover:bg-green-700 focus:outline-none dark:focus:ring-green-800">Edit</a>
